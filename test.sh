...
[ -f tmp/test-$t.md5 ] || {
  md5sum tests/pom$t.xml tests/pom$t-*.xml 2> /dev/null > tmp/test-$t.md5.tmp ||:
  mv tmp/test-$t.md5.tmp tmp/test-$t.md5
}

md5sum tests/pom$t.xml tests/pom$t-*.xml 2> /dev/null > tmp/test-$t.md5.test.tmp ||:
mv tmp/test-$t.md5.test.tmp tmp/test-$t.md5.test
...
[ $collect = 1 ] && {
    mvn dependency:collect -f tests/pom$t.xml > tmp/test-$t.coll.tmp || {
      echo "test $t failed"
      exit 1
    }
    mv tmp/test-$t.coll.tmp tmp/test-$t.coll
...
# deps.py
python deps.py tests/pom$t.xml --sections deps -m $module | grep '^ ' | sed 's/^ *//' > tmp/test-$t.deps.tmp && mv tmp/test-$t.deps.tmp tmp/test-$t.deps
